...
function randomInput(worksheetName) {
  ...
  if (isValidRow(excelRows[0])) {
    ...
    submitBut.onclick = () => {
      generateStudents(excelRows, input.value);
    };
...
function generateStudents(rows, no) {
  ...
  document.getElementsByName("yearCheck").forEach((check) => {
    if (check.checked == true) years.push(parseInt(check.value));
  });

  rows.forEach((row) => {
    if (studentFilter(row, years)) accepted.push(row);
  ...
  if (no > accepted.length) {
    tableDiv.innerHTML =
      "<h3>Needed students greater than students available. Only have " +
      accepted.length +
      " students</h3>";
    for (var x = 0; x < accepted.length; x++)
      randomlySelected.push(accepted.pop());
  } else {
    for (var x = 0; x < no; x++) randomlySelected.push(accepted.pop());
  }
